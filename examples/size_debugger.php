<?php

declare(strict_types=1);

use BitWasp\Bitcoin\Transaction\TransactionFactory;
use BitWasp\Buffertools\Buffertools;

require __DIR__ . "/../vendor/autoload.php";

if ($argc > 1) {
    $txHex = $argv[1];
} else {
    $txHex = "";
}

$fields = [];
$tx = TransactionFactory::fromHex($txHex);
$fields[] = [4, "version {$tx->getVersion()}"];
$fields[] = [Buffertools::numToVarInt(count($tx->getInputs()))->getSize(), 'nIn'];

foreach ($tx->getInputs() as $input) {
    $script = $input->getScript();
    $scriptSize = $script->getBuffer()->getSize();
    $scriptVarInt = Buffertools::numToVarInt($scriptSize);
    $fields[] = [32, "\ttxid\t".$input->getOutPoint()->getTxId()->getHex()];
    $fields[] = [4, "\tvout\t".$input->getOutPoint()->getVout()];
    $fields[] = [$scriptVarInt->getSize(), "\tvarint\t" . $scriptVarInt->getHex()];
    $fields[] = [$scriptSize, "\tscript\t".$input->getScript()->getHex()];
    $fields[] = [4, "\tseq\t".$input->getSequence()];
}
$fields[] = [Buffertools::numToVarInt(count($tx->getOutputs()))->getSize(), 'nOut'];
foreach ($tx->getOutputs() as $output) {
    $script = $output->getScript();
    $scriptSize = $script->getBuffer()->getSize();
    $scriptVarInt = Buffertools::numToVarInt($scriptSize);
    $fields[] = [8, "\tvalue\t{$output->getValue()}"];
    $fields[] = [$scriptVarInt->getSize(), "\tvarint {$scriptVarInt->getHex()}\t"];
    $fields[] = [$scriptSize, "\tscript\n{$script->getHex()}"];
}
$fields[] = [4, "lockTime {$tx->getLockTime()}"];

$totalIn = 0;
foreach ($fields as $field) {
    $totalIn += $field[0];
    echo "{$field[0]}\t{$field[1]}\n";
}
echo "$totalIn\tTOTAL\n";
