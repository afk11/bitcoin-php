...
class BitcoinGoldSigHash
{
     ...
     * @throws \Exception
     */
    public function calculate(ScriptInterface $txOutScript, $inputToSign, $sigVersion, $sighashType = SigHash::ALL)
    {
        $forkHashType = (int) $sighashType;
        if (($sighashType & 0x40) > 0) {
            $forkHashType |= $this->forkId;
        }

        if ($sigVersion === 1 || ($sighashType & 0x40)) {
        ...
        $sigHashData = new Buffer($serializer->serializeTransaction() . pack('V', $forkHashType));
